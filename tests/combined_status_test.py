...
def test_combined_status_old_last_block(mock_daemon, mock_time_range, mock_monero_rpc, caplog):
    ...
    assert "block_timestamp" in response[LAST_BLOCK_KEY]["error"]
    assert "hash" in response[LAST_BLOCK_KEY]["error"]
    ...
    assert response[LAST_BLOCK_KEY]["error"]["block_timestamp"] == "2019-12-20T07:55:33", "Wrong error."
    assert response[LAST_BLOCK_KEY]["error"]["hash"] == "3f82c93e6f7726a54724d0b8b1026bec878af449bc2f97e9a916c6af72a6367a", "Wrong error."
