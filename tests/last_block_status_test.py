...
def test_last_block_not_recent_timestamp_old(mock_time_range, mock_monero_rpc, caplog):
    ...
    assert "block_timestamp" in response["error"]
    assert "hash" in response["error"]
    ...
    assert response["error"]["block_timestamp"] == "2019-12-20T07:55:33", "Wrong error."
    assert response["error"]["hash"] == "3f82c93e6f7726a54724d0b8b1026bec878af449bc2f97e9a916c6af72a6367a", "Wrong error."
